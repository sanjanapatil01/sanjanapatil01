<!-- Banner -->
<img src="https://github.com/sanjanapatil01/sanjanapatil01/blob/main/learner.png" width="100%" />

<h2 align="center">👋 Hi, I'm Sanjana Patil</h2>

<h1 align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&pause=950&center=true&vCenter=true&width=600&lines=MERN+Stack+Developer;Java+Backend;AI+%26+ML+Learner;Building+with+logic+and+creativity..." />
</h1>

<p align="center">
  <a href="mailto:sanjanampatil6@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  <a href="https://www.linkedin.com/in/sanjana-m-patil-a66074325/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://github.com/sanjanapatil01" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white" />
  </a>
</p>

---

## 🚀 About Me

- 💻 Passionate MERN Stack Developer focused on building scalable applications  
- 🔧 Skilled in Java DSA logic   
- 🧠 Strong analytical thinking — I believe in solving problems, not just writing code  
- 🤖 Curious about AI and ML and how they shape future technologies  


---

## 🛠️ Tech Skills

| Category | Technologies |
|----------|--------------|
| **Frontend**  | HTML, CSS, Bootstrap, JavaScript, React.js |
| **Backend**   | Node.js, Express.js |
| **Database**  | MongoDB |
| **Programming** | Java, Python |
| **Machine Learning** | Regression, Classification, XGBoost, Model Evaluation |
| **AI & APIs** | Gemini API, REST APIs, JWT Auth, WebRTC |
| **Tools** | GitHub, VS Code, Postman, Docker (basic) |

---



## 📊 GitHub Analytics

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=sanjanapatil01&show_icons=true&theme=radical" height="150" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=sanjanapatil01&layout=compact&theme=radical" height="150" />
</p>



<h2 align="center"><i>Dream in logic. Live in code</i></h2>

